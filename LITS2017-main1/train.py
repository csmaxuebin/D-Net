...
训练算法
def train(args, train_loader, model, criterion, optimizer, epoch,timestamp):
    acc=0
    sen = 0
    accs = AverageMeter()
    sens = AverageMeter()
...
def main():
    ...
    # model = Unet.UNet_2Plus(args)
    # model = Unet.UNet_2Plus()
    ...
    log = pd.DataFrame(index=[], columns=[
        'epoch', 'lr', 'loss', 'iou','dice_1', 'dice_2', 'acc','sen'])
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train_log = train(args, train_loader, model, criterion, optimizer, epoch,timestamp)

        # scheduler.step()
        # evaluate on validation set
        # val_log = validate(args, val_loader, model, criterion)

        print('loss %.4f - iou %.4f - dice_1 %.4f - dice_2 %.4f'%(train_log['loss'], train_log['iou'], train_log['dice_1'], train_log['dice_2']))
        ...
        tmp = pd.Series([
            ...
            train_log['acc'],
            train_log['sen'],
        ], index=['epoch', 'lr', 'loss', 'iou', 'dice_1' ,'dice_2' ,'acc','sen'])
        ...
        torch.save(model.state_dict(),
                   'models/{}/{}/epoch{}_model.pth'.format(args.name, timestamp, epoch))
