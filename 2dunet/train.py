...
from dataset1.dataset import CTDataset
...
# print("11111111111111",sys.argv[1])
# CFG_FILE = sys.argv[1]
CFG_FILE = "cfgs/trainval.yaml"
计算准确率
...
log = pd.DataFrame(index=[], columns=['epoch', 'acc','sen','loss', 'iou'])
...
for e in range(RESUME_EPOCH, TRAIN_EPOCH):
    ...
    for i, (all_F, all_M, all_info) in enumerate(TrainLoader):
        ...
        sen = sensitivity(preds,labels)
        miou = torch.mean(CalcIoU(preds, labels)).item()

        rT += time.time()-tik
        Epoch_CE.write(loss); Epoch_mIOU.write(miou);
        tmp = pd.Series([e,acc,sen,loss.item(),miou,], index=['epoch','accuracy', 'sensitivity','loss', 'iou'])
        log = log.append(tmp,ignore_index=True)
        log.to_csv('./save/log.csv',index=False)
        #break
